...
/**
 * Ward stock, as panels rather than a page.
 *
 * This is the GENERAL ledger — WardStock + WardStockLedger — and it has always
 * worked for every drug. The NDPS screens were narrower duplicates of it that
 * refused anything not flagged as a narcotic, which is why they were unusable
 * for ordinary stock.
 *
 * Split into panels so the stock-transfer board can mount them as its own tabs
 * with ONE ward chosen at the top, rather than nesting a second set of tabs
 * inside the first.
 */
...
/** Every ward that holds stock. The choice is lifted so both panels share it. */
export function useWardOptions() {
  const { data: wards } = useWards();
  return (wards ?? []) as Array<{ id: string; name: string }>;
}

/** What the ward holds now, and the actions on a line. */
export function WardStockPanel({ wardId }: { wardId: string }) {
...
/** Every movement of this ward's stock — received, dispensed, returned, adjusted. */
export function WardLedgerPanel({ wardId }: { wardId: string }) {
