...
// ============================================================
// IP Medication Indents — WARD NURSE view (design doc I).
// The nurse raises indents for admitted patients, tracks them through the
// pharmacy lifecycle, and acknowledges receipt once the batch is delivered
// (verifying the delivered medication against the indent before administering).
...
const LEGEND = ['raised', 'approved', 'dispensed', 'delivered', 'acknowledged'];
...
export default function NurseIndentsPage() {
  ...
  const rows = useMemo(() => {
    let r = wardFilter ? all.filter((i) => i.wardId === wardFilter) : all;
    ...
    return r;
  }, [all, wardFilter, search]);

  const [raiseOpen, setRaiseOpen] = useState(false);
  const [viewTarget, setViewTarget] = useState<MedicationIndent | null>(null);
  const [ackTarget, setAckTarget] = useState<MedicationIndent | null>(null);
    ...
    <div className="space-y-4 animate-fade-in-up">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Request IP medication from pharmacy and acknowledge receipt once delivered to the ward.
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 rounded-lg border bg-card px-3 py-2">
        ...
        {LEGEND.map((s, i) => (
          <span key={s} className="flex items-center gap-1.5">
            ...
            {i < LEGEND.length - 1 && <span className="text-muted-foreground">→</span>}
          </span>
        ))}
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="w-full sm:w-56">
          <Select
            value={wardFilter || NONE}
            onValueChange={(v: string | null) => setWardFilter(v === NONE ? '' : v ?? '')}
          >
            <SelectTrigger>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      No indents yet. Raise one to request medication from pharmacy.
                    </p>
                  </td>
                </tr>
              ) : (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-center gap-1">
                        {(ind.status === 'dispensed' || ind.status === 'delivered') && (
                          <Button
                            size="sm"
                            variant="ghost"
                            className="text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50"
                            onClick={() => setAckTarget(ind)}
                          >
      ...
      {/* Dialogs */}
      <RaiseIndentDialog open={raiseOpen} onOpenChange={setRaiseOpen} defaultWardId={wardFilter} />
      {ackTarget && (
        <AcknowledgeIndentDialog indent={ackTarget} onClose={() => setAckTarget(null)} />
      )}
      {cancelTarget && (
...
// ============================================================
// Acknowledge receipt dialog — verify delivered batch, then acknowledge all
// ============================================================

function AcknowledgeIndentDialog({
  indent,
  onClose,
}: {
  ...
  const ack = useAcknowledgeIndent();
  ...
  const submit = () => {
    ack.mutate(
      { id: indent.id },
      {
        ...
        onError: (err: any) => toast.error(err?.message ?? 'Failed to acknowledge receipt'),
      },
    );
  };
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Acknowledge receipt · {indent.indentNumber}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3 py-1">
          <p className="text-sm text-muted-foreground">
            Verify the delivered medication for <b>{indent.patientName ?? 'the patient'}</b> against
            the indent below. Acknowledging confirms the batch was received on the ward and is ready
            to administer.
          </p>
                ...
                {(indent.items ?? []).map((it: MedicationIndentItem) => (
                  <tr key={it.id} className="border-b last:border-b-0">
                    ...
                    <td className="px-3 py-2 text-center">
                      {it.dispensedQty ?? it.approvedQty ?? it.requestedQty}
                    </td>
                    <td className="px-3 py-2 capitalize">
                      {it.saleUnit === 'loose' ? it.looseUnitLabel || 'loose' : it.saleUnit}
                    </td>
                  </tr>
                ))}
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={ack.isPending}>
            {ack.isPending && <Loader2 className="mr-1.5 h-4 w-4 animate-spin" />}
            Acknowledge Receipt
          </Button>
        </DialogFooter>
      </DialogContent>
        ...
        <div className="space-y-3 py-1">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={3}
              placeholder="e.g. order changed, duplicate request..."
        ...
        <div className="w-28 shrink-0">
          ...
          >
            ...
            <SelectContent>
              ...
              {!isTto && <SelectItem value="loose">{line.looseUnitLabel || 'Loose'}</SelectItem>}
            </SelectContent>
          </Select>
        </div>
